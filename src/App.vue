    ...
    <div id="maize">
      <table>
        <tr v-for="(row, y) in mazeArray" :key="y">
          <td v-for="(cell, x) in row" :key="x" :class="getCellClass(x, y)" @click="setStart"
      ...
      <p>
        通路の部分を左クリックでスタート地点を、右クリックでゴール地点を設定できます。
      </p>
    </div>
  </main>
</template>
...
export default {
  ...
  methods: {
    ...
    async mazeDig() {
      ...
      while (stack.length > 0) { // スタックが空になるまで繰り返す
        if (this.isMaking === false) return;
    ...
    getDigNext(x, y) {
      ...
      for (let direction of directions) {
        ...
        if (nextX > 0 && nextX < this.width - 1 && nextY > 0 && nextY < this.height - 1 && this.mazeArray[nextY][nextX] === 1) {
